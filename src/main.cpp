...
// Use pin D5 for the wind speed pin to have the onboard SCL LED light up whenever there is a HIGH signal on D9.
WindSensor windSensor(150, messageInterval, D5, D10, D11, D12, D13);
...
void loop() {
  ...
  if (haveMillisPassed(messageInterval)) {
    ...
    StaticJsonDocument<300> payload;  // TODO optimize byte size to actual size of JSON contents
    ...
    payload["gas"]["acetone"] = String(gasReadings.at("C3H6O"));
    payload["gas"]["ammonium"] = String(gasReadings.at("NH4"));
    payload["gas"]["carbonDioxide"] = String(gasReadings.at("CO2"));
    payload["gas"]["carbonMonoxide"] = String(gasReadings.at("CO"));
    payload["gas"]["ethanol"] = String(gasReadings.at("C2H6O"));
    payload["gas"]["toluene"] = String(gasReadings.at("C7H8"));
